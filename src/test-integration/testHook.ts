...
//   ollamaCode._test.exec    — relay a {__test__} op to the webview, await its result
//   ollamaCode._test.post    — inject a raw HostToWebview message (the fake event stream)
// (the extension also registers ollamaCode._test.openPanel, which opens a chat
// panel and calls attachTestWebview below so these commands have a live target.)
//
...
export function registerTestCommands(context: vscode.ExtensionContext): void {
  context.subscriptions.push(
    vscode.commands.registerCommand('ollamaCode._test.exec', (op: Record<string, unknown>) =>
      exec(op),
    ),
    // Inject a raw host->webview message — used to feed the fake event stream.
    vscode.commands.registerCommand('ollamaCode._test.post', (msg: unknown) => {
      if (!activeWebview) {
