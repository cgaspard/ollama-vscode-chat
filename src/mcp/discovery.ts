 ...
 *   4. `ollamaCode.mcpServers`      — our own explicit override   (map or {servers})
 *
...
function readJson(file: string): unknown {
  ...
  try {
    ...
    vscode.window.showWarningMessage(
      `Ollama Code: ${vscode.workspace.asRelativePath(file)} is not valid JSON; its MCP servers were ignored.`,
    );
    return undefined;
  }
}
...
export function discoverMcpServers(): McpDiscovery {
  ...
  try {
    sources.explicit = vscode.workspace.getConfiguration('ollamaCode').get('mcpServers');
  } catch {
    // absent — fine
  }
